...
“频段偏好”会先读取并按当前设备保存原始 LTE 掩码。日常使用建议保留多个频段，只把长期实测表现较差的频段排除；单选一个 Band 属于实验性单频锁定。写入后程序会检查网络注册、分组数据附着和实际驻留频段，75 秒内未恢复可用网络时自动写回修改前的 LTE 掩码。页面中的“恢复自动选频”会恢复该设备首次修改前保存的配置。这里限制的是模块允许搜索的 LTE Band，不会锁定具体基站、PCI 或 EARFCN。

页面会把模块允许的频段分为“中国大陆常用 4G”和“其他地区 / 漫游频段”，并标注中国移动、中国联通和中国电信的常见 LTE 归属。中国广电自身获许可的 700MHz 在国内主要用于 5G；对于本项目的 4G 模块，广电卡实际可能通过中国移动共享网络接入移动 LTE 频段，因此页面将移动频段额外标为“广电共享”候选，并以当前真机驻留结果为准。频段归属依据工信部截至 2024 年 4 月公布的公众移动通信频段统计；本地部署、网络优化和 SIM 策略仍可能造成差异。
