...
| 模式 | 页面名称 | 主要用途 |
| --- | --- | --- |
| 模式 0 | 管理兼容模式 | 保留 AT、短信和 eSIM 管理，用于 USB 网络兼容性排查 |
| 模式 1 | 日常模式 | 同时提供 USB 4G 上网、短信收发、AT 指令和 eSIM 管理 |
...
短信功能不要求切换到模式 0。手头的 QDC507 已真机确认在 `usbnet=1` 日常模式下同时保留 USB 网卡和 AT bulk 接口，程序可以一边通过 `en19` 上网，一边每 8 秒轮询新短信，并继续提供 eSIM Profile 和 AT 指令管理。只有模块或 macOS USB 网卡出现兼容问题时，才需要临时切换模式 0 排查。
